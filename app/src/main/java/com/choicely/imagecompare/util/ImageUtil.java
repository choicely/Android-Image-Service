package com.choicely.imagecompare.util;
...
import com.choicely.imageservice.ImageChooser;
import com.choicely.imageservice.ImageServiceListener;
...
import com.choicely.imagecompare.fresco.FrescoImageVH;
import com.choicely.imagecompare.list.ImageVH;
...
//
    ...
    private final String PREFERENCE_SAMPLE_SIZE = "sample_size";
    private final String PREFERENCE_BLUR = "blur";
    ...
    private final ChoicelyImageService is;
    ...
    public enum ImageServiceProvider {
        ...
        CHOICELY_IS(R.id.settings_choicely_image_service),;
    ...
    private ImageUtil(Context ctx) {
        ...
        imageSampleSize = preferences.getInt(PREFERENCE_SAMPLE_SIZE, 1000);
        blur = preferences.getBoolean(PREFERENCE_BLUR, false);
...
//                .writeDebugLogs()
        ...
        is = ChoicelyImageService.getInstance();
    }
    ...
    public void setImageSamplseSize(Integer size) {
        imageSampleSize = size;
        if (size != null) {
            preferences.edit().putInt(PREFERENCE_SAMPLE_SIZE, size).apply();
        } else {
            preferences.edit().putInt(PREFERENCE_SAMPLE_SIZE, 0).apply();
        }
    }
    ...
    public void setBlur(boolean blur) {
        this.blur = blur;
        preferences.edit().putBoolean(PREFERENCE_BLUR, blur).apply();
    }
    ...
    private void clearChoicelyCache() {
        is.deleteFiles();
    }
...
//                setFrescoImage(holder, url);
            ...
            case UNIVERSAL_IMAGE_LOADER:
                ...
                break;
            case CHOICELY_IS:
                setChoicelyImage(holder, url);
                break;

        }
    }

    private void setChoicelyImage(final ImageVH holder, String url) {
        final long start = System.currentTimeMillis();
        ImageChooser info = new ImageChooser(url)
                .sampleSize(imageSampleSize)
                .setBlur(blur)
                .setEnterAnimation(AnimationUtils.loadAnimation(context, R.anim.fade_in))
                .setImageServiceListener(new ImageServiceListener() {
                    @Override
                    public void imageReady(String imageUrl, Bitmap image, int defaultResId) {
                        onImageReady(holder.text, imageUrl, start);
                        if (image != null) {
                            QLog.d(TAG, "Image size[%d, %d]", image.getWidth(), image.getHeight());
                        }
                    }
...
//        holder.image.setImageURI(uri);
        ...
        glide.load(url)
                ...
                .listener(new RequestListener<String, GlideDrawable>() {
                    ...
                    public boolean onResourceReady(GlideDrawable resource, String model, Target<GlideDrawable> target, boolean isFromMemoryCache, boolean isFirstResource) {
                        onImageReady(holder.text, url, start);
                        QLog.d(TAG, "ImageView size[%d, %d]", holder.image.getWidth(), holder.image.getHeight());
                        QLog.i(TAG, "Glide onResourceReady res[%d, %d]", resource.getIntrinsicWidth(), resource.getIntrinsicHeight());
    ...
    private void setPicassoImage(final ImageVH holder, final String url) {
        ...
        picasso.load(url)
                ...
                .resize(imageSampleSize, imageSampleSize) // if image too large app crashes!
                .onlyScaleDown()
